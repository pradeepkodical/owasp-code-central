<%@ Control Language="c#" AutoEventWireup="false" Codebehind="manageHosts.ascx.cs" Inherits="sourceControl.controls.manageHosts" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>


<br>

<h1>Manage Hosts Allow</h1>
<br>

This allows you to restrict access to the system to specific Ip addresses. <br>
Note the useHostsAllow key must be set to true to use this functionality.
...
<asp:Panel ID=panelNew Visible=False Runat=server>
	...
	<table width=500>
		...
		<tr>
			<td> IP: </td>
			<td> &nbsp; </td>
			<td> <asp:TextBox ID="txtIp" Runat=server Width=300></asp:TextBox></td>
	...
	<ItemStyle CssClass="even" />
				<Columns>
					...
					<asp:TemplateColumn ItemStyle-Width="50" >
						<ItemTemplate>
							<asp:Image BorderWidth=0 Runat=server ImageUrl="~/common/images/host.gif" ID="Image1" NAME="Image1"></asp:Image>
							
						</ItemTemplate>
				
					</asp:TemplateColumn>
					
					<asp:TemplateColumn ItemStyle-Width="300" HeaderText="Description">
						<ItemTemplate>
						&nbsp;
							<asp:linkButton ID="lblDescription" text='<%# DataBinder.Eval(Container.DataItem, "description") %>' CommandName=cmdEdit Font-Underline=True CssClass="normal" Width=260 Runat=server />
						</ItemTemplate>
				
					</asp:TemplateColumn>
					
					<asp:TemplateColumn ItemStyle-Width="300" HeaderText="Ip">
						<ItemTemplate>
							<asp:label ID="lblIp" text='<%# DataBinder.Eval(Container.DataItem, "ip") %>' CssClass="normal" Width=260 Runat=server />
						</ItemTemplate>
