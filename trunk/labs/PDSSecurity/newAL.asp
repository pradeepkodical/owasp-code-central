...
function SubmitForm() {
  ...
  txt = document.forms.ALAdd.UsedIDs.value;
  start = 1;
  finish = 1;
  index = 0;
  ...
  for (i = 0; i < UsedIDs.length; i++) {
    if (document.forms.ALAdd.ALName.value.toLowerCase() == UsedIDs[i].toLowerCase())
      Used = 1;
  }

  if (Used)
    window.alert("This access level name has already been used.  Please type in a unique name.");
  else if (document.forms.ALAdd.ALName.value == "")
    window.alert("Enter a name first.");
  else {
    if (window.confirm('You are about to add the access level "' + document.forms.ALAdd.ALName.value + '". Access Levels cannot be deleted. Are you sure you want to proceed?')) {
      document.forms.ALAdd.submit();
      window.alert("Access Level added.");
...
<FORM name="ALAdd" METHOD="post" target="tabhidden" action="addAL.asp">
...
<TABLE border=0 width=80% height=100% align=center valign=center>
  <tr><td height=30%>
    ...
    <INPUT type=text size=25 name="ALName" value="">
    ...
    <%rst.close%>
    <INPUT onclick="SubmitForm();" class="SelectButton" name="Submit" type="Button" value="Add This Access Level">
  </td></tr>
