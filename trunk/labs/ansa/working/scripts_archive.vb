...
''''			Session("Safe_mode") = False	
		...
		' MISC
			Session("External_URLs_to_test") = "www.google.com:80:Google website (www.google.com)," + _
							   "www.bbc.co.uk:80:BBC website (www.bbc.uk)," + _
							   "www.microsoft.uk:80:Microsoft (www.microsoft.com)," + _
							   "127.0.0.1:80:Local Computer"
		
			Session("Internal_URLs_to_test") = "127.0.0.1:80:local Computer (127.0.0.1)," + _
							   "127.0.0.1:80:local Computer (127.0.0.1)," + _
							   "127.0.0.1:80:local Computer (127.0.0.1)"

			Session("Internal_ADSL_URLs_to_test") = "10.0.0.2:80:local Computer (10.0.0.2)," + _
							   	"10.0.0.15:80:local Computer (10.0.0.15)," + _
							   	"10.0.0.16:80:local Computer (10.0.0.16)"
...
'						td_link("center","More details",mapped_path ,"_blank")
	...
	Public sub Run_Security_tests(List_of_tests_to_run)
		...
		Dim tests_to_run , test, mapped_path, web_data, test_result, split_value
...
'			response.write(web_data)
			if instr(web_data, Session("http_signature")) then
				try
					test_result = split(web_data,";")
					split_value=split(test_result(2),",")
					Update_field(split_value(1)+test,"name",b_(test))
					Update_field(split_value(1)+test,"result",test_result(1))
					Update_field(split_value(1)+test,"classification",test_result(2))
					Update_field(split_value(1)+test,"comments",test_result(3))
					Update_field(split_value(1)+test,"more_details",link_("More details",mapped_path+"?mode=interactive" ,"_blank"))
				catch
					Update_field(split_value(1)+test,"result","Test Error (array)")
				end try
			else
				Update_field(split_value(1)+test,"result","Test Error")
			end if
			response.flush		
