...
namespace Owasp.VulnReport.ascx
{
	...
	public class ascxXsltEditor : System.Windows.Forms.UserControl
	{
		...
		private void btFindText_Click(object sender, System.EventArgs e)
		{
            ...
            int iOriginalTextCaret = textEditorControl.ActiveTextAreaControl.TextArea.Caret.Offset ;
            int iOffsetOfEndOfCurrentSelection = textEditorControl.ActiveTextAreaControl.TextArea.Caret.Offset + 
                 textEditorControl.ActiveTextAreaControl.TextArea.SelectionManager.SelectedText.Length;
      
            int iFoundPos = textEditorControl.Text.IndexOf(txtTextToFind.Text, iOffsetOfEndOfCurrentSelection);  //start from the cursor position
            if (iFoundPos == -1) // didn't find anything
                iFoundPos = textEditorControl.Text.IndexOf(txtTextToFind.Text, 0); // start from the top
			...
			{				
                ...
                textEditorControl.ActiveTextAreaControl.Caret.Position = textEditorControl.ActiveTextAreaControl.TextArea.SelectionManager.SelectionCollection[0].StartPosition;
            ...
            if (iOriginalTextCaret == textEditorControl.ActiveTextAreaControl.TextArea.Caret.Offset)
            {
                lbPhraseNotFound.Visible = true;
