...
namespace beretta.Objects
{
	...
	public class urlWorker
	{
		...
		private DataSet objPayloadDataSet;
		private DataSet objSignaturesDataSet;
		private formSubmitter objFormSubmitter=new formSubmitter();
		private urls mObjUrl=new urls();
		...
		public session objSession
		{
			get{return mObjSession;}
			set{mObjSession=value;}
		}
		...
		public void authenticate()
		{
			...
			if (mAuthenticationType != 0)
			{
				...
				{
					sessionFormsLogon objSessionFormsLogon=new sessionFormsLogon();
					objSessionFormsLogon.sessionId=objSession.id;
					objSessionFormsLogon.populateBySessionId();

					objFormSubmitter.login(objSessionFormsLogon.totalFormSubmission, objSessionFormsLogon.loginPageUrl);



				}
					//Raw Submission
				else if (mAuthenticationType==2)
				{
					sessionRawLogon objSessionRawLogon=new sessionRawLogon();
					objSessionRawLogon.sessionId=objSession.id;
					objSessionRawLogon.populateBySessionId();

					objFormSubmitter.login(objSessionRawLogon.rawSubmission, objSessionRawLogon.loginPageUrl);
