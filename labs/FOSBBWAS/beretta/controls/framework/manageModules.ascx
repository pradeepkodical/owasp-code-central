<%@ Control Language="c#" AutoEventWireup="false" Codebehind="manageModules.ascx.cs" Inherits="sourceControl.controls.manageModules" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
...
<asp:Panel ID=panelNew Visible=False Runat=server>
	...
	<table width=500>
		...
		<tr>
			<td> Src: </td>
			<td> &nbsp; </td>
			<td> <asp:TextBox ID=txtSrc Runat=server Width=300></asp:TextBox></td>
	...
	<ItemStyle CssClass="even" />
				<Columns>
					...
					<asp:TemplateColumn ItemStyle-Width="50" >
						<ItemTemplate>
							<asp:Image BorderWidth=0 Runat=server ImageUrl="~/common/images/module.gif" ID="Image1" NAME="Image1"></asp:Image>
							
						</ItemTemplate>
				
					</asp:TemplateColumn>
					
					<asp:TemplateColumn ItemStyle-Width="300" HeaderText="Src">
						<ItemTemplate>
						&nbsp;
							<asp:linkButton ID="lblUsername" text='<%# DataBinder.Eval(Container.DataItem, "src") %>' CommandName=cmdEdit Font-Underline=True CssClass="normal" Width=260 Runat=server />
						</ItemTemplate>
					...
					<asp:TemplateColumn ItemStyle-Width="300" HeaderText="Description">
						<ItemTemplate>
							<asp:label ID="lblKeyValue" text='<%# DataBinder.Eval(Container.DataItem, "description") %>' CssClass="normal" Width=260 Runat=server />
						</ItemTemplate>
