<%@ Control Language="c#" AutoEventWireup="false" Codebehind="managePasswords.ascx.cs" Inherits="beretta.Web.managePasswords" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<br>
<h1>Manage Passwords
</h1>
		...
		<td>
			...
			<table width="700" class="searchPanel">
				<tr width="700" class="searchPanel">
					<td width="0"></td>
					<td width="0"></td>
					<td width="350"><span class="datagridSearchText">Password:</span></td>
					<td width="50"></td>
				</tr>
				<tr width="700" class="searchPanel">
					<td width="0"></td>
			...
			<asp:datagrid id="dbgrid" runat="server" Width="700" OnItemCommand="dgCommand_onClick" PagerStyle-Mode="NumericPages"
				...
				<Columns>
					...
					<asp:TemplateColumn HeaderText="Password" ItemStyle-Width="300">
						<ItemTemplate>
							<asp:textbox ID="txtExistingValue" Width=100 MaxLength=50 text='<%# DataBinder.Eval(Container.DataItem, "password") %>' CssClass="normaltextbox" Runat=server />
						</ItemTemplate>
						<FooterTemplate>
					...
					<asp:TemplateColumn HeaderText="Item Order" ItemStyle-Width="300">
						<ItemTemplate>
							<asp:textbox ID="txtExistingOrder" Width=100 MaxLength=50 text='<%# DataBinder.Eval(Container.DataItem, "passwordOrder") %>' CssClass="normaltextbox" Runat=server />
						</ItemTemplate>
						<FooterTemplate>
