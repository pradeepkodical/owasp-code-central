...
<body>
 ...
 <blockquote>
   ...
   <p>Although in some scenarios this is a valid solution in some cases, for 
   ...
   releases a patch for this problem, the ISPs will have to execute their 
   client's website code with 'Full trust'.</p>
   <p>Given the dangers and risks posed by situation (see Security Analyser), 
   then the only other solution is prevention and detection.</p>
...
&nbsp;</b><ul>
       <li>Create a tool that executes after all compilation is done (IL code is 
       created)<b> </b>but before the it is executed by the OS (when IL is 
       transformed in local machine code and executed)<b>. </b>This toll would 
       validate the DLL/EXE and approve/disapprove it execution based on the 
       calls that are made. For example if a DLL has a call to the Win32's &quot;winExec&quot;&nbsp; 
       function<b> </b>then that (unless the DLL has a special permission) its 
       execution will be denied.</li>
       <li>This tool could be used on 'live' websites or could be used offline 
       (some of this might have to be developed my Microsoft). The offline mode 
       ...
       (download from the web, in an email, copied to disk) before they are 
       executed by the system. The AV companies should develop such products 
       because they already have the core engine&nbsp; the automatic updates, 
...
&nbsp;<ul>
       ...
       <li>hability to rebuid a server in 1h:<ul>
         ...
         <li>Assign IPs</li>
         <li>decommision old server (if still live) and publish rebuilt server 
         without any loss of service<br>
&nbsp;</li>
       </ul>
       </li>
       <li>idealy situation would be to have the servers in a cluster 
       environment where if one server is compromized then it can be removen 
...
&nbsp;</li>
     ...
     designed for such scenario. Another possibiliy would be to use Linux which 
     will be possible once the porting of the .Net platform is fully completed.<br>
&nbsp;</li>
     <li><b>Audit Website Folders and Files:</b><br>
&nbsp;<ul>
       <li>monitor (audit) all files that belog each user (NTFS with logs stored 
       in the Event Viewer)</li>
       <li>create custom reports per site / per user with details of who (from 
...
&nbsp;<ul>
       ...
       <li>Validate identity using Credit Card system<br>
&nbsp;</li>
     </ul>
     </li>
